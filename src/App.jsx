...
const buildCSS = () => `@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); *,*::before,*::after{box-sizing:border-box;margin:0;padding:0} html,body{font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text','Plus Jakarta Sans',sans-serif;background:${C.ice};color:${C.text};font-size:14px;-webkit-font-smoothing:antialiased} input,select,textarea,button{font-family:inherit} .shell{display:flex;min-height:100vh} .sidebar{width:232px;flex-shrink:0;background:linear-gradient(180deg,${C.deepTeal} 0%,${C.midTeal} 100%);display:flex;flex-direction:column} .main-area{flex:1;overflow-y:auto;padding:28px 30px;min-height:100vh} .sb-logo{display:flex;align-items:center;gap:10px;padding:22px 18px 14px} .sb-logo-icon{width:36px;height:36px;background:rgba(0,180,216,.22);border-radius:10px;display:flex;align-items:center;justify-content:center} .sb-logo-title{color:#fff;font-weight:700;font-size:15px;letter-spacing:-.01em} .sb-logo-sub{color:rgba(255,255,255,.45);font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.05em} .sb-user{margin:0 10px 6px;background:rgba(255,255,255,.09);border-radius:12px;padding:10px 12px} .sb-user-name{color:#fff;font-weight:600;font-size:13px} .sb-user-id{color:rgba(255,255,255,.45);font-size:11px;margin-top:2px} .sb-div{height:1px;background:rgba(255,255,255,.08);margin:6px 0} .sb-nav{padding:0 8px;display:flex;flex-direction:column;gap:2px} .sb-item{display:flex;align-items:center;gap:9px;padding:9px 12px;border-radius:10px;font-size:13px;font-weight:600;color:rgba(255,255,255,.6);cursor:pointer;border:none;background:transparent;width:100%;text-align:left;transition:all .15s} .sb-item:hover{background:rgba(255,255,255,.09);color:#fff} .sb-item.active{background:rgba(0,180,216,.3);color:#fff} .sb-spacer{flex:1} .sb-bottom{padding:8px 8px 16px} .card{background:${C.white};border-radius:16px;box-shadow:0 1px 3px rgba(15,23,42,.06);border:1px solid ${C.borderLight};padding:20px} .card-0{background:${C.white};border-radius:16px;box-shadow:0 1px 3px rgba(15,23,42,.06);border:1px solid ${C.borderLight};overflow:hidden} .page-title{font-size:20px;font-weight:700;color:${C.text};margin-bottom:3px;letter-spacing:-.01em} .page-sub{font-size:13px;color:${C.muted};margin-bottom:22px} .sec-title{font-size:14px;font-weight:600;color:${C.text};margin-bottom:14px} .label{display:block;font-size:11px;font-weight:600;color:${C.muted};text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px} .req{color:${C.red};margin-left:2px} .input{width:100%;padding:10px 13px;border:1px solid ${C.borderLight};border-radius:10px;font-size:13px;color:${C.text};background:${C.grey0};outline:none;transition:border .15s,background .15s,box-shadow .15s} .input:focus{border-color:${C.cyan};background:${C.white};box-shadow:0 0 0 3px rgba(0,180,216,.13)} .input::placeholder{color:${C.border}} .input-auto{border-color:${C.cyan};background:${C.cyanLight};color:${C.deepTeal}} .input-ro{background:${C.ice};color:${C.muted};cursor:default} .btn{display:inline-flex;align-items:center;gap:6px;padding:9px 18px;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;border:none;transition:all .15s} .btn-cyan{background:${C.cyan};color:#fff}.btn-cyan:hover{background:#0099bb} .btn-red{background:${C.red};color:#fff;width:100%;justify-content:center;padding:13px;font-size:14px} .btn-outline{background:transparent;border:1px solid ${C.cyan};color:${C.cyan}}.btn-outline:hover{background:${C.cyanLight}} .btn-ghost{background:${C.grey0};color:${C.muted};border:1px solid ${C.borderLight}}.btn-ghost:hover{background:${C.grey1}} .btn-sm{padding:5px 11px;font-size:11px;border-radius:8px} .btn:disabled{opacity:.38;cursor:not-allowed;pointer-events:none} .alert{padding:10px 14px;border-radius:10px;font-size:13px;font-weight:500;margin-bottom:14px} .alert-err{background:${C.redLight};color:${C.red};border-left:3px solid ${C.red}} .alert-ok{background:${C.greenLight};color:${C.green};border-left:3px solid ${C.green}} .alert-info{background:${C.cyanLight};color:${C.midTeal};border-left:3px solid ${C.cyan}} .alert-warn{background:${C.orangeLight};color:${C.orange};border-left:3px solid ${C.orange}} .badge{display:inline-block;padding:3px 9px;border-radius:20px;font-size:11px;font-weight:600} .badge-cyan{background:${C.cyanLight};color:${C.midTeal}} .badge-green{background:${C.greenLight};color:${C.green}} .badge-orange{background:${C.orangeLight};color:${C.orange}} .badge-red{background:${C.redLight};color:${C.red}} .badge-purple{background:${C.purpleLight};color:${C.purple}} .badge-grey{background:${C.grey1};color:${C.muted}} .tbl{width:100%;border-collapse:collapse;font-size:13px} .tbl th{background:${C.grey0};color:${C.muted};font-size:10.5px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;padding:10px 14px;text-align:left} .tbl td{padding:11px 14px;border-bottom:1px solid ${C.borderLight};vertical-align:middle} .tbl tr:last-child td{border-bottom:none} .tbl tr:hover td{background:${C.grey0}} .g2{display:grid;grid-template-columns:1fr 1fr;gap:14px} .g3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px} .g4{display:grid;grid-template-columns:repeat(4,1fr);gap:14px} .col2{grid-column:1/-1} .stack{display:flex;flex-direction:column;gap:18px} .stack-sm{display:flex;flex-direction:column;gap:10px} .flex-b{display:flex;justify-content:space-between;align-items:center} .flex-g{display:flex;gap:10px;align-items:center;flex-wrap:wrap} .tab-row{display:flex;border-bottom:2px solid ${C.borderLight};margin-bottom:18px} .tab-btn{padding:9px 18px;font-size:13px;font-weight:600;cursor:pointer;border:none;background:transparent;color:${C.muted};border-bottom:2px solid transparent;margin-bottom:-2px;transition:all .15s} .tab-btn.active{color:${C.cyan};border-bottom-color:${C.cyan}} .upload-zone{border:1.5px dashed ${C.border};border-radius:14px;padding:28px;text-align:center;cursor:pointer;transition:all .15s} .upload-zone:hover,.upload-zone.drag{border-color:${C.cyan};background:${C.cyanLight}} .cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:4px} .cal-header{text-align:center;font-size:10px;font-weight:600;color:${C.muted};text-transform:uppercase;padding:4px 0} .cal-day{border-radius:12px;padding:7px 4px;text-align:center;border:1px solid ${C.borderLight};transition:all .15s;min-height:52px} .cal-day.today{border-color:${C.cyan};box-shadow:0 0 0 2px rgba(0,180,216,.25)} .cal-day.off-day{background:${C.grey0};opacity:.65} .cal-day.work-day{background:${C.white};cursor:pointer}.cal-day.work-day:hover{border-color:${C.cyan};background:${C.cyanLight}} .cal-day.selected{border-color:${C.cyan};background:${C.cyanLight}} .cal-day.empty{border-color:transparent;background:transparent;min-height:0} .cal-day-num{font-size:13px;font-weight:700;color:${C.text}} .cal-shift-chip{font-size:9px;font-weight:600;padding:2px 6px;border-radius:6px;line-height:1.4;margin-top:3px;display:inline-block} .radio-group{display:flex;flex-direction:column;gap:8px} .radio-opt{display:flex;align-items:center;gap:10px;padding:10px 14px;border:1px solid ${C.borderLight};border-radius:12px;cursor:pointer;transition:all .15s;background:${C.white}} .radio-opt:hover{border-color:${C.cyan};background:${C.cyanLight}} .radio-opt.sel{border-color:${C.cyan};background:${C.cyanLight};color:${C.deepTeal};font-weight:600} .radio-opt.disabled{opacity:.4;cursor:not-allowed;pointer-events:none} .radio-dot{width:17px;height:17px;border-radius:50%;border:2px solid ${C.border};flex-shrink:0;display:flex;align-items:center;justify-content:center} .radio-dot.on{border-color:${C.cyan};background:${C.cyan}} .radio-inner{width:6px;height:6px;border-radius:50%;background:#fff} .addr-card{border:1px solid ${C.borderLight};border-radius:14px;padding:14px;cursor:pointer;transition:all .15s;background:${C.white}} .addr-card:hover{border-color:${C.cyan}} .addr-card.sel{border-color:${C.cyan};background:${C.cyanLight}} .route-badge{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:600;background:${C.cyanLight};color:${C.midTeal};margin-top:6px}
 ...
 @media(max-width:768px){
  ...
  .col2{grid-column:1!important}
  .page-title{font-size:17px}
  .page-sub{font-size:12px;margin-bottom:14px}
  .card{padding:14px;border-radius:12px}
  .card-0{border-radius:12px}
  .sec-title{font-size:13px}
  .input{padding:13px 14px;font-size:15px;border-radius:10px}
  .btn{padding:11px 16px;font-size:13px}
  .btn-red{padding:16px;font-size:15px;border-radius:12px}
  .btn-sm{padding:7px 12px;font-size:12px}
  .radio-opt{padding:13px 14px;border-radius:10px;font-size:14px}
  .radio-dot{width:20px;height:20px}
  .radio-inner{width:8px;height:8px}
  ...
  .cal-day{min-height:44px;padding:5px 2px}
  .cal-day-num{font-size:12px}
  .cal-shift-chip{font-size:8px;padding:1px 4px}
 ...
 @media(max-width:768px){
  .mob-header{display:flex;align-items:center;justify-content:space-between;padding:0 16px;height:56px;background:linear-gradient(90deg,${C.deepTeal} 0%,${C.midTeal} 100%);position:sticky;top:0;z-index:200;flex-shrink:0}
  .mob-header-left{display:flex;align-items:center;gap:10px}
  .mob-header-title{color:#fff;font-weight:800;font-size:15px}
  .mob-header-sub{color:rgba(255,255,255,.5);font-size:10px;font-weight:600}
  .mob-header-user{text-align:right}
  .mob-header-name{color:#fff;font-weight:700;font-size:12px}
  .mob-header-id{color:rgba(255,255,255,.45);font-size:10px}
  .mob-nav{display:flex;position:fixed;bottom:0;left:0;right:0;height:62px;background:${C.deepTeal};z-index:200;border-top:1px solid rgba(255,255,255,.1)}
  .mob-nav-item{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;cursor:pointer;border:none;background:transparent;color:rgba(255,255,255,.45);font-size:10px;font-weight:700;font-family:'Plus Jakarta Sans',sans-serif;transition:color .15s;padding:0}
  .mob-nav-item.active{color:${C.cyan}}
  .mob-quick-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px}
  .mob-quick-btn{background:${C.white};border-radius:14px;padding:18px 14px;text-align:center;cursor:pointer;border:1.5px solid ${C.grey1};box-shadow:0 2px 8px rgba(13,61,86,.07);transition:all .15s;display:flex;flex-direction:column;align-items:center;gap:8px}
  .mob-quick-btn:active{transform:scale(.97)}
  .mob-quick-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center}
  .mob-quick-label{font-size:13px;font-weight:700;color:${C.text}}
  .mob-quick-sub{font-size:11px;color:${C.muted};font-weight:500}
  .mob-greeting{background:linear-gradient(135deg,${C.deepTeal} 0%,${C.midTeal} 100%);border-radius:16px;padding:20px;margin-bottom:14px;color:#fff}
  .mob-greeting-name{font-size:18px;font-weight:800;margin-bottom:3px}
  .mob-greeting-sub{font-size:12px;opacity:.7}
  .mob-today-card{background:rgba(0,180,216,.15);border-radius:10px;padding:12px 14px;margin-top:12px;display:flex;align-items:center;justify-content:space-between}
 }`;
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={d} className={cls} onClick={() => isWork && onSelectDate && onSelectDate(dateStr, entry)}>
              <div className="cal-day-num" style={{ color: isOff ? C.muted : isSel ? C.deepTeal : isToday ? C.cyan : C.text }}>{d}</div>
              {entry && (
...
// Everything below is additive — it does not modify EmployeeShell,
// AdminDashboard, TransportForm, RosterPage, ProfilePage, or any DB/Session/
// design-token helper above. It reuses them as-is. The ROOT App() component
// below picks MobileEmployeeShell vs EmployeeShell based on useMobile().
// ════════════════════════════════════════════════════════════════════════════

// ─── useMobile hook ────────────────────────────────────────────────────────
// Returns true when the viewport is under `breakpoint` px wide, and stays in
// sync as the window is resized or rotated. Safe to call once, at the top of
// any component; call it unconditionally near the top of App().
      ...
      <div className="m-header">
        <div>
          <div className="m-header-title">
            {screen === "home" ? `Hi, ${user.name.split(" ")[0]}` : MOBILE_TITLES[screen]}
          </div>
          {screen === "home" && <div className="m-header-sub">{user.empId}</div>}
        </div>
        {screen !== "home" && (
          <button
...
// ════════════════════════════════════════════════════════════════════════════
// HOME — quick actions + scrollable submissions list
// ════════════════════════════════════════════════════════════════════════════
      ...
      <div className="m-card"><div className="m-card-pad">
        ...
        {form.route ? (
          ...
          <div style={{ marginTop: 10 }}>
            <label className="m-label">Route</label>
            <select className="m-input" value={form.route} onChange={e => set("route", e.target.value)}>
              <option value="">— Select route —</option>
              {ROUTES.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
...
// ════════════════════════════════════════════════════════════════════════════
// MY ROSTER — upload + calendar (reuses parseRosterExcel & MonthCalendar as-is)
// ════════════════════════════════════════════════════════════════════════════
function MobileRoster({ user, onUserUpdate }) {
  const [selYear, setSelYear] = useState(nowYear());
  const [selMonth, setSelMonth] = useState(nowMonth());
  const [msg, setMsg] = useState(null);
  const [preview, setPreview] = useState(null);
  ...
  const processFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        const wb = XLSX.read(e.target.result, { type: "binary", cellDates: false });
        const { result, detectedMonth } = parseRosterExcel(wb, selYear, selMonth);
        if (detectedMonth) { setSelMonth(detectedMonth.month); setSelYear(detectedMonth.year); }
        const myData = result[user.empId];
        if (!myData || Object.keys(myData).length === 0)
          return setMsg({ t: "err", m: `No column found for your ID (${user.empId}).` });
        const count = Object.keys(myData).length;
        const workDays = Object.values(myData).filter(v => !v.shiftInfo?.off).length;
        const usedMonth = detectedMonth ? detectedMonth.month : selMonth;
        const usedYear = detectedMonth ? detectedMonth.year : selYear;
        setPendingData({ data: myData, month: usedMonth, year: usedYear });
        setPreview({ count, workDays });
        setMsg({ t: "ok", m: `Found ${count} day(s) (${workDays} working). Tap Import to confirm.` });
      } catch (err) {
        setMsg({ t: "err", m: "Could not read file: " + err.message });
      }
    };
  ...
  const importData = async () => {
    ...
    onUserUpdate(updated);
    setPendingData(null); setPreview(null);
    setMsg({ t: "ok", m: `${Object.keys(myData).length} day(s) imported for ${MONTHS[useMonth - 1]} ${useYear}.` });
  };
    ...
    <div>
      ...
      <div className="m-card"><div className="m-card-pad">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ fontWeight: 800, fontSize: 15 }}>{MONTHS[selMonth - 1]} {selYear}</div>
          {Object.keys(rosterMonth).length > 0 && (
            <div style={{ display: "flex", gap: 6 }}>
...
function MobileProfile({ user, onUpdate, onLogout, theme, onThemeChange }) {
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      const code = String(Math.floor(100000 + Math.random() * 900000));
      const expiry = Date.now() + 10 * 60 * 1000;
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, { to_email: user.email, to_name: user.name, otp_code: code }, EMAILJS_PUBLIC_KEY);
      setPwOtpCode(code); setPwOtpExpiry(expiry); setPwOtp(""); setPwStep("verify");
      setPwMsg({ t: "ok", m: `Code sent to ${maskEmail(user.email)}.` });
    } catch (e) {
      setPwMsg({ t: "err", m: "Failed: " + e.message });
    }
    setPwLoading(false);
  ...
  const verifyPwOtp = async () => {
    if (!pwOtp) return setPwMsg({ t: "err", m: "Enter the code sent to your email." });
    if (pwOtp !== pwOtpCode) return setPwMsg({ t: "err", m: "Incorrect code." });
    if (Date.now() > pwOtpExpiry) return setPwMsg({ t: "err", m: "Code expired. Please start over." });
    setPwLoading(true);
    try {
      const { error } = await supabase.auth.updateUser({ password: newPw });
  ...
  return (
    <div>
      ...
      <div className="m-card"><div className="m-card-pad">
        <div className="m-summary-row"><span className="m-summary-label">Name</span><span className="m-summary-val">{user.name}</span></div>
        <div className="m-summary-row"><span className="m-summary-label">Employee ID</span><span className="m-summary-val">{user.empId}</span></div>
        <div className="m-summary-row"><span className="m-summary-label">Email</span><span className="m-summary-val">{user.email || "Not set"}</span></div>
      </div></div>
      ...
      <div className="m-card"><div className="m-card-pad">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <label className="m-label" style={{ margin: 0 }}>Saved Addresses ({addrs.length}/3)</label>
          <button
            onClick={openNew} disabled={addrs.length >= 3}
            style={{ background: C.cyan, color: "#fff", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 700, fontSize: 12, minHeight: 32 }}>
            + Add
          </button>
        </div>
        {editing !== null && (
          <div style={{ background: "var(--m-surface2)", border: `1.5px solid ${C.cyan}`, borderRadius: 12, padding: 12, marginBottom: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <select className="m-input" value={af.route} onChange={e => setAf(p => ({ ...p, route: e.target.value }))}>
              <option value="">— Select route —</option>
              {ROUTES.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
        ...
        {addrs.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {addrs.map((addr, i) => (
              <div key={addr.id} className="m-option" style={{ alignItems: "flex-start" }}>
                <div className="m-option-icon" style={{ background: C.cyanLight }}><Ico n="pin" s={16} c={C.cyan} /></div>
                <div style={{ flex: 1 }}>
